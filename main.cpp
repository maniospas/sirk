...
typedef struct
{
    ...
    int amount;        // damage value
    float timer;       // lifetime
    bool active;
    bool isPlayer;
} DamageEvent;
#define DrawText(text, posX, posY, fontSize, color) \
    DrawTextEx(gameFont, text, (Vector2){(float)(posX), (float)(posY)}, (float)(fontSize), 2, color)
#define MeasureText(text, fontSize) \
    MeasureTextEx(gameFont, text, (float)(fontSize), 2).x
...
bool EmptyTile(int x, int y)
{
    if (!Inside(x, y))
        return false;
    if (grid[y][x] == TILE_WALL || grid[y][x] == TILE_PILLAR)
        return false;
    if (EnemyAt(x, y))
        return false;
...
Enemy enemyTemplates[] =
{
    ...
    {0,0, 3,0,3, 3, true,  false, ENEMY_FOOD},
};
...
void InitGrid()
{
    ...
    int pillarCount = 0; // adjust density here
...
void DrawGame()
{
    ...
    if(luckcontrols)
        DrawStat(texTR, TextFormat("Gold %d", player.treasure), 16+140*3, 16);
    ...
    for (int i = 0; i < enemyCount; i++)
    {
        ...
        DrawTexturePro(
            ...
            (Rectangle){(float)(enemies[i].x * TILE),
                        (float)(enemies[i].y * TILE + STATUS_BAR_SIZE),
                        (float)TILE,
                        (float)TILE},
            (Vector2){0,0},
            0,
        ...
        // --- Segmented HP pips (1 pip = 2 HP, max 20 HP → 10 pips) ---
        int hp = enemies[i].hp;
        if(hp < 0) hp = 0;
        int pips = (hp + 1) / 2;   // ceil(hp / 2)
        int pipWidth  = 4;
        int pipHeight = 6;
        int spacing   = 2;
        // center inside tile
        ...
        // "-18" lifts them slightly so they appear above the character center

        for(int p = 0; p < pips; p++)
        {
            int px = startX + p * (pipWidth + spacing);
            DrawRectangle(px, startY, pipWidth, pipHeight, (Color){120, 0, 0, 255});
        }
    ...
    //draw player
    {
        ...
        DrawTexturePro(
            ...
            (Rectangle){(float)(player.x * TILE),
                        (float)(player.y * TILE + STATUS_BAR_SIZE),
                        (float)TILE,
                        (float)TILE},
            (Vector2){0,0},
            0,
    ...
    for(int i = 0; i < MAX_DAMAGE_EVENTS; i++)
    {
        ...
        if(damageEvents[i].amount > 0)
            DrawText(TextFormat("%d", -damageEvents[i].amount),
                    px - 12,
                    py,
                    38,
                    Fade((Color){120, 0, 0, 255}, alpha));
        else
            DrawText(TextFormat("+%d", damageEvents[i].amount),
                        px - 12,
                        py,
                        38,
                        Fade((Color){0, 120, 0, 255}, alpha));
    }
    ...
    for (int i = 0; i < HAND_SIZE; i++)
    {
        ...
        DrawScaledDistorted(texCard,
                baseX,
                baseY+10,
                PREVIEW_SIZE, PREVIEW_SIZE*3/2);

        if(hand[i] == ENEMY_NONE)
        {

            // --- Key label ---
            DrawText("ENTER to buy",
                    baseX + padding+4,
                    baseY - 18,
                    24,
                    YELLOW);
        ...
        DrawScaled(GetEnemyTexture(hand[i]),
                baseX + padding,
                baseY + padding,
                portraitSize);
        DrawText(TextFormat("%d to spawn", i+1),
                baseX + padding+4,
                baseY - 18,
                24,
                YELLOW);
        ...
        // ATK (top-right inside stat row)
        if(def.atk > 0)
        {
            DrawScaled(texATT,
                    baseX + PREVIEW_SIZE - padding - iconSize - 24,
                    statY,
                    iconSize);

            DrawText(TextFormat("%d", def.atk),
                    baseX + PREVIEW_SIZE - padding - 24,
                    statY,
                    24,
                    WHITE);
        }
        ...
        if(def.hp > 0)
        {
            ...
            DrawText(TextFormat("%d", def.hp),
                    baseX + padding + iconSize + 4,
                    statY,
                    24,
                    WHITE);
        ...
        if(def.treasure > 0)
        {
            ...
            DrawText(TextFormat("%d", def.treasure),
                    baseX + padding + iconSize + 4,
                    statY + 2,
                    24,
                    GOLD);
        ...
        if(def.food > 0)
        {
            ...
            DrawText(TextFormat("%d", def.food),
                    baseX + padding + iconSize + 4,
                    statY + 2,
                    24,
                    GOLD);
